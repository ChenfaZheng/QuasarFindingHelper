...
LastEditors: chenfa
LastEditTime: 2021-07-19 02:08:07
'''
...
def get_sources(fid: str, source_range: list) -> pd.DataFrame:
    ...
    return raw[source_range[0]:source_range[1]]
...
def image_finder(source: pd.Series, source_id: int =None) -> None:
    ...
    # plt.tight_layout()
    if source_id == None:
        plt.savefig('./results/%04d.png'%(source['order']))
    else:
        plt.savefig('./results/%04d_%04d.png'%(source_id, source['order']))
    plt.close()
    pass
...
def main() -> None:
    ...
    if not os.path.exists('./results'):
        os.mkdir('./results')
    job_range = [0, 80]
    sources = get_sources('./613compact.xlsx', job_range)
    print(sources)
    source_ids = list(range(job_range[0]+1, job_range[1]+1))
    for sid, source in sources.iterrows():
        image_finder(source, source_ids[sid])
    pass
